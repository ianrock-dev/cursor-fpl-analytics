...
import { FaSearch, FaSpinner, FaUser, FaExclamationTriangle, FaFlask } from 'react-icons/fa';
import { fetchManagerById, getMockManagerById } from '@/utils/api';
...
export default function ManagerSearch() {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (searchType === 'id') {
        ...
        const result = await fetchManagerById(Number(searchTerm));
        ...
        if (!result.success) {
          ...
          if (isSpecificUser) {
            ...
            const mockData = getMockManagerById(searchTerm);
  ...
  const useDemoData = async () => {
    ...
    if (searchTerm === '598864') {
      const specificData = getMockManagerById('598864');
      demoManagers.push({
        id: 598864,
      ...
      const mockData = getMockManagerById(searchTerm || '123456');
      demoManagers.push({
        id: mockData.id,
        playerName: mockData.player_first_name + ' ' + mockData.player_last_name,
        teamName: mockData.name,
    ...
    <div className="dark:bg-dark-secondary rounded-lg shadow-lg p-6">
      ...
      <div className="mb-8 max-w-2xl mx-auto">
        ...
        <form onSubmit={handleSearch} className="space-y-4">
            ...
            <div className="flex-1">
              ...
              <div className="relative">
                ...
                <input
                  type="text"
                  id="searchTerm"
                  className={`w-full pl-10 pr-4 py-2 rounded bg-gray-800 border border-gray-700 text-white focus:border-fpl-green focus:outline-none ${
                    isDemoMode ? 'opacity-50' : ''
                  }`}
                  placeholder={searchType === 'id' ? "Enter manager ID (e.g., 1234567)" : "Enter team name"}
                  value={searchTerm}
          ...
          <button
            type="submit"
            className="bg-fpl-dark hover:bg-opacity-90 text-white font-medium py-2 px-6 rounded focus:outline-none focus:ring-2 focus:ring-fpl-green"
            disabled={loading || (isDemoMode ? false : !searchTerm)}
          >
            {loading ? (
              ...
              isDemoMode ? 'Refresh Demo Data' : 'Search'
            )}
          </button>
        </form>
        
        {error && (
          <div className="mt-4 p-3 bg-red-900 bg-opacity-30 border border-red-700 rounded-md text-red-400 flex items-center">
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-700">
              {managers.map((manager) => (
                <tr key={manager.id} className="bg-gray-800 bg-opacity-40 hover:bg-opacity-70 transition-colors">
                  <td className="px-6 py-4 font-medium">
                    {manager.teamName}
                    {(isDemoMode || manager.id === 598864) && (
                      <span className="ml-2 text-xs text-amber-400 italic">
