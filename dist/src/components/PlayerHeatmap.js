...
function PlayerHeatmap({ playerId, type }) {
    const [loading, setLoading] = (0, react_1.useState)(true);
    const [player, setPlayer] = (0, react_1.useState)(null);
    const [heatmapData, setHeatmapData] = (0, react_1.useState)([]);
    const [matches, setMatches] = (0, react_1.useState)([]);
    const [error, setError] = (0, react_1.useState)(null);
    const [isPlaceholder, setIsPlaceholder] = (0, react_1.useState)(false);
    const [statsBombAttribution, setStatsBombAttribution] = (0, react_1.useState)(false);
    const [isRealData, setIsRealData] = (0, react_1.useState)(false);
    ...
    (0, react_1.useEffect)(() => {
        if (!canvasRef.current || heatmapData.length === 0 || loading)
            return;
    ...
    const drawPitch = (ctx, width, height) => {
        ...
        // Draw center circle
        ctx.beginPath();
        ctx.arc(xOffset + pitchWidth / 2, yOffset + pitchHeight / 2, pitchHeight / 10, 0, Math.PI * 2);
        ctx.stroke();
        // Draw penalty areas
        ...
        // Left penalty area
        ctx.strokeRect(xOffset, yOffset + penAreaYOffset, penAreaWidth, penAreaHeight);
        // Right penalty area
        ctx.strokeRect(xOffset + pitchWidth - penAreaWidth, yOffset + penAreaYOffset, penAreaWidth, penAreaHeight);
        // Draw goal areas
        const goalAreaWidth = pitchWidth / 10;
        const goalAreaHeight = pitchHeight / 5;
        const goalAreaYOffset = (pitchHeight - goalAreaHeight) / 2;
        // Left goal area
        ctx.strokeRect(xOffset, yOffset + goalAreaYOffset, goalAreaWidth, goalAreaHeight);
        // Right goal area
        ctx.strokeRect(xOffset + pitchWidth - goalAreaWidth, yOffset + goalAreaYOffset, goalAreaWidth, goalAreaHeight);
        // Draw penalty spots
        const penSpotDistance = pitchWidth / 8;
        // Left penalty spot
        ctx.beginPath();
        ctx.arc(xOffset + penSpotDistance, yOffset + pitchHeight / 2, 2, 0, Math.PI * 2);
        ctx.fillStyle = '#ffffff';
        ctx.fill();
        // Right penalty spot
        ctx.beginPath();
        ctx.arc(xOffset + pitchWidth - penSpotDistance, yOffset + pitchHeight / 2, 2, 0, Math.PI * 2);
        ctx.fill();
    };
    // Function to draw direction arrow (left to right)
    const drawDirectionArrow = (ctx, width, height) => {
    ...
    const drawHeatmap = (ctx, data, width, height) => {
        ...
        for (const point of data) {
            ...
            const gradient = tempCtx.createRadialGradient(canvasX, canvasY, 0, canvasX, canvasY, radius);
    ...
    if (loading) {
        return (<div className="bg-gray-800 rounded-lg p-2 animate-pulse h-20 max-w-xs mx-auto">
    ...
    if (error) {
        return (<div className="bg-gray-800 rounded-lg p-2 h-20 max-w-xs mx-auto">
    ...
    return (<div className={`relative bg-gray-900 rounded-lg overflow-hidden mx-auto ${type === 'season' ? 'max-w-sm' : 'max-w-xs'}`}>
      {/* Header */}
      <div className="p-2 bg-gray-800 border-b border-gray-700">
        ...
        {!isRealData && (<div className="mt-1 text-xs text-gray-400 flex items-center">
      ...
      {/* Loading state */}
      {loading && (<div className="flex justify-center items-center h-64 bg-gray-900">
          <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-blue-500"></div>
        </div>)}
      
      {/* Error state */}
      {error && !loading && (<div className="flex justify-center items-center h-64 bg-gray-900 text-red-500 p-4 text-center">
          {error}
        </div>)}
      
      {/* Canvas for heatmap */}
      {!loading && !error && (<div className="relative">
          <canvas ref={canvasRef} width={type === 'season' ? 300 : 240} height={type === 'season' ? 200 : 160} className="w-full h-auto"/>
        </div>)}
      ...
      {statsBombAttribution && (<div className="text-xs text-gray-400 text-right mt-1">
          Data provided by StatsBomb
        </div>)}

      {/* Recent matches section */}
      {type === 'fixtures' && matches.length > 0 && (<div className="mt-4">
          ...
          <div className="space-y-3">
            ...
            {Object.entries(matches.reduce((acc, match) => {
                ...
                return acc;
            }, {})).map(([competition, competitionMatches]) => (<div key={competition} className="mb-3">
                <h5 className="text-xs font-medium text-gray-300 mb-1">{competition}</h5>
                <div className="grid grid-cols-1 gap-1">
                  {competitionMatches.map((match) => (<div key={match.id} className="bg-gray-700 rounded p-2 flex items-center justify-between">
                      ...
                      <div className="ml-3 flex flex-col items-center">
                        ...
                        <div className="flex items-center text-xs text-gray-400 mt-1">
                          {match.goals > 0 && (<div className="flex items-center mr-2">
                              <fa_1.FaFutbol className="mr-1"/>
                              <span>{match.goals}</span>
                            </div>)}
                          {match.assists > 0 && (<div className="flex items-center">
                              <fa_1.FaHandsHelping className="mr-1"/>
...
function getPositionColor(elementType) {
    switch (elementType) {
        case 1: // GK
...
function getPositionStyle(elementType) {
    switch (elementType) {
        case 1: // GK
