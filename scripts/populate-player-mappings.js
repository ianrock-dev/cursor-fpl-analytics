...
async function getAllStatsBombPlayers() {
  try {
    ...
    for (const competition of premierLeagueCompetitions) {
      try {
        const matches = await fetchStatsBombData(
          `/matches/${competition.competition_id}/${competition.season_id}.json`
        );
        ...
        console.warn(
          `Error fetching matches for competition ${competition.competition_id}, season ${competition.season_id}:`,
          error.message
        );
      }
    }
